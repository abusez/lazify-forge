...
public class ColorUtil {
    ...
    public static boolean hasObfuscationBefore(String formatted, String name) {
        ...
        boolean[] magicAt = new boolean[256];
        int visLen = 0;
        ...
        for (int i = 0; i < formatted.length(); i++) {
            ...
            if (c == '\u00a7' && i + 1 < formatted.length()) {
                char code = Character.toLowerCase(formatted.charAt(i + 1));
                if (code == 'k') magic = true;
                else if (code == 'r' || "0123456789abcdef".indexOf(code) >= 0) magic = false;
                i++;
                continue;
            }
            if (visLen >= magicAt.length) {
                boolean[] grown = new boolean[magicAt.length * 2];
                System.arraycopy(magicAt, 0, grown, 0, magicAt.length);
                magicAt = grown;
            }
            visible.append(c);
            magicAt[visLen++] = magic;
        }

        String vis = visible.toString();
        String lowerVis = vis.toLowerCase();
        String lowerName = name.toLowerCase();
        int idx = lowerVis.indexOf(lowerName);
        if (idx < 0) return false;

        if (idx > 0 && magicAt[idx - 1]) return true;
        for (int j = idx; j < idx + name.length() && j < visLen; j++) {
            if (magicAt[j]) return true;
        }
        return false;
    }
    ...
    public static String getPingColor(int ping) {
        ...
        if (ping < 150) return "\u00a7e" + ping;
        if (ping < 200) return "\u00a76" + ping;
        return "\u00a7c" + ping;
    }

    public static String getWinstreakColor(String winstreak) {
        if (winstreak == null || winstreak.isEmpty()) return "";
        String numPart = winstreak;
        String suffix = "";
        int space = winstreak.indexOf(' ');
        if (space > 0) {
            numPart = winstreak.substring(0, space);
            suffix = winstreak.substring(space + 1);
        }
        try {
            int v = Integer.parseInt(numPart);
            if (v == 0) return "";
            String colored = colorWinstreakValue(v);
            if (suffix.isEmpty()) return colored;
            return colored + " " + colored.substring(0, 2) + suffix;
        } catch (NumberFormatException e) {
            return "\u00a77" + winstreak;
        }
    }

    private static String colorWinstreakValue(int v) {
    ...
    public static String getUrchinTagColor(String tagType) {
        if (tagType == null || tagType.isEmpty()) return "";
        String norm = tagType.toLowerCase().replace(' ', '_');
        switch (norm) {
            case "blatant_cheater":
            case "blatant_cheating":  return "\u00a7cBC";
            case "confirmed_cheater":
            case "confirmed_cheating": return "\u00a7cC";
            case "closet_cheater":
            case "closet_cheating":   return "\u00a7eCC";
            case "sniper":            return "\u00a74S";
            default:
                if (norm.contains("blatant")) return "\u00a7cBC";
                if (norm.contains("confirmed")) return "\u00a7cC";
                if (norm.contains("closet")) return "\u00a7eCC";
                if (norm.contains("sniper")) return "\u00a74S";
                return "";
        }
    }

    /** Seraph {@code report_type} → overlay abbreviation (matches Urchin where possible). */
    public static String getSeraphTagColor(String reportType) {
        if (reportType == null || reportType.isEmpty()) return "";
        String normalized = reportType.toLowerCase().replace(' ', '_');
        String mapped = getUrchinTagColor(normalized);
