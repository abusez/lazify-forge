...
public class GuiOverlayPosition extends GuiScreen {
    ...
    private int scaledOverlayW() {
        return Math.max(1, Math.round(overlayW * previewScale()));
    }

    private int scaledOverlayH() {
        return Math.max(1, Math.round(overlayH * previewScale()));
    }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (dragging) {
            ...
            // Clamp to screen (account for scale)
    ...
    private void drawPreviewOverlay() {
        ...
        boolean scaled = OverlayRenderer.pushScale(scale, overlayX, overlayY);
        try {
            drawPreviewOverlayInner();
        } finally {
            OverlayRenderer.popScale(scaled);
        }
    }

    private void drawPreviewOverlayInner() {
        Minecraft mc = Minecraft.getMinecraft();
        if (mc.fontRendererObj == null) return;
